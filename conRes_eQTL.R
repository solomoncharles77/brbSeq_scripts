...
untrCands <- read.delim("resFiles/geneExprUNTR_brbSeq_cisEQTL_topAssocs.txt", header = F)
colnames(untrCands) <- c("geneID", "geneName", "var_id")
...
tnfaCands <- read.delim("resFiles/geneExprTNFA_brbSeq_cisEQTL_topAssocs.txt", header = F)
colnames(tnfaCands) <- c("geneID", "geneName", "var_id")
...
sigM <- compareModels[compareModels$term != "h0", ]
sigM2 <- sigM[sigM$p.value < 0.05, ]
